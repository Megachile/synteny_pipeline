...
def create_locus_matrix(
    ...
    locus_targets = targets_df[
        (targets_df[COL_PLACEMENT] == PLACEMENT_SYNTENY) &
        (targets_df[COL_ASSIGNED_TO] == locus_id)
    ]
...
def create_novel_locus_matrix(
    locus_id: str,
    locus_name: str,
    member_genomes: List[str],
    empty_genomes: List[str],
    novel_flanking_annotations: Dict[str, Dict],
    species_map: Dict[str, str],
    phylo_order_map: Dict[str, int],
    seq_metadata: Dict,
) -> pd.DataFrame:
    """
    Create matrix for a novel locus.

    Since novel loci don't have BK reference flanking genes, we show:
    - Member status (has target, empty synteny only, no match)
    - Representative protein length
    - Top flanking gene SwissProt annotations (from Phase 7)
    """
    matrix_rows = []

    # Get unique flanking genes from annotations (top ones by frequency)
    flanking_by_genome = {}
    for protein_id, info in novel_flanking_annotations.items():
        candidate_id = info.get('candidate_id', '')
        sp_name = info.get('swissprot_name', '')

        # Map to genome - protein_id format: Genome_Scaffold_Gene.1
        for genome in member_genomes + empty_genomes:
            if protein_id.startswith(genome + '_'):
                if genome not in flanking_by_genome:
                    flanking_by_genome[genome] = []
                if sp_name and pd.notna(sp_name):
                    flanking_by_genome[genome].append(str(sp_name))
                break
    ...
    for genome in sorted(species_map.keys()):
        ...
        # Novel locus membership status
        if genome in member_genomes:
            row['status'] = 'MEMBER'
            row['has_target'] = 'Y'
        elif genome in empty_genomes:
            row['status'] = 'EMPTY_SYNTENY'
            row['has_target'] = '-'
        else:
            row['status'] = 'NO_MATCH'
            row['has_target'] = ''

        # Get length from seq_metadata if available
        row['length_aa'] = ''
        for target_id, meta in seq_metadata.items():
            if target_id.startswith(genome + '_') and f'NOVEL_{locus_name}' in str(meta.get('locus', '')):
                row['length_aa'] = meta.get('length_aa', '')
                break

        # Top flanking gene annotations (unique)
        genome_flanking = flanking_by_genome.get(genome, [])
        unique_flanking = list(dict.fromkeys(genome_flanking))[:5]  # Up to 5 unique
        row['flanking_annotations'] = '; '.join(unique_flanking) if unique_flanking else ''
...
def main():
    ...
    parser.add_argument("--phase5b-dir", type=Path, default=None,
                        help="Phase 5b novel loci directory (for novel loci matrices)")
    ...
    if syntenic_file.exists():
        ...
        required_cols = ['genome', 'target_gene_id', COL_PLACEMENT, COL_ASSIGNED_TO]
        try:
            validate_required_columns(targets_df, required_cols, "syntenic_targets.tsv")
    ...
    # Phase 2b synteny blocks (try new filename first, then legacy for backward compatibility)
    empty_blocks_df = pd.DataFrame()
    if args.phase2b_dir:
        phase2b_blocks_file = args.phase2b_dir / FILE_PHASE2B_SYNTENY_BLOCKS
        if not phase2b_blocks_file.exists():
            phase2b_blocks_file = args.phase2b_dir / FILE_EMPTY_SYNTENY_BLOCKS_LEGACY
    ...
    # =========================================================================
    # Generate Novel Loci Matrices (from Phase 5b)
    # =========================================================================
    novel_loci_count = 0

    if args.phase5b_dir and args.phase5b_dir.exists():
        novel_clustered_file = args.phase5b_dir / "novel_loci_clustered.tsv"
        novel_flanking_annot_file = args.phase7_dir / "novel_loci_flanking_annotated.tsv" if args.phase7_dir else None

        if novel_clustered_file.exists():
            print("\n[3] Generating novel loci matrices...")

            # Load novel loci clusters
            novel_clusters_df = pd.read_csv(novel_clustered_file, sep='\t')
            print(f"  Novel loci: {len(novel_clusters_df)}")

            # Load novel loci flanking annotations from Phase 7
            novel_flanking_annotations = {}
            if novel_flanking_annot_file and novel_flanking_annot_file.exists():
                annot_df = pd.read_csv(novel_flanking_annot_file, sep='\t')
                for _, row in annot_df.iterrows():
                    protein_id = row['helixer_gene_id']
                    novel_flanking_annotations[protein_id] = {
                        'candidate_id': row.get('candidate_id', ''),
                        'swissprot_name': row.get('swissprot_name', ''),
                        'swissprot_pident': row.get('swissprot_pident', 0),
                    }
                print(f"  Novel flanking annotations: {len(novel_flanking_annotations)}")

            # Generate matrix for each novel locus
            for _, locus_row in novel_clusters_df.iterrows():
                locus_id = locus_row['locus_id']
                locus_name = locus_row['locus_name']
                member_genomes = locus_row.get('member_genomes', '').split(';') if pd.notna(locus_row.get('member_genomes', '')) else []
                empty_genomes = locus_row.get('empty_genomes', '').split(';') if pd.notna(locus_row.get('empty_genomes', '')) else []

                matrix_df = create_novel_locus_matrix(
                    locus_id, locus_name,
                    member_genomes, empty_genomes,
                    novel_flanking_annotations,
                ...
                if not matrix_df.empty:
                    output_file = args.output_dir / f"{args.family}_NOVEL_{locus_name}_matrix.tsv"
                    matrix_df.to_csv(output_file, sep='\t', index=False)

                    n_members = len(matrix_df[matrix_df['status'] == 'MEMBER'])
                    n_empty = len(matrix_df[matrix_df['status'] == 'EMPTY_SYNTENY'])
                    print(f"  NOVEL_{locus_name}: {n_members} members, {n_empty} empty synteny")
                    novel_loci_count += 1

            if novel_loci_count == 0:
                print("  No novel loci matrices generated")
    else:
        print("\n[3] Skipping novel loci (no Phase 5b directory)")
    ...
    if novel_loci_count > 0:
        print(f"  Including {novel_loci_count} novel loci matrices")
