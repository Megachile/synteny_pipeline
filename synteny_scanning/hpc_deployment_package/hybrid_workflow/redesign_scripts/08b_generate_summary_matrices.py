...
def parse_args():
    ...
    p.add_argument('--exclude-genomes-file', type=Path, help='File with genome IDs to exclude (one per line, # comments)')
    return p.parse_args()
...
def main():
    ...
    # Pass through exclusion file if provided
    if args.exclude_genomes_file:
        cmd.extend(['--exclude-genomes-file', str(args.exclude_genomes_file)])
