...
# Dynamically calibrated maximum gene span based on actual gene coordinates
# This replaces the hardcoded 10kb limit that was filtering out valid genes
CALIBRATED_MAX_GENE_SPAN_KB: float = 10.0  # Default, will be updated from actual data


def load_gene_coordinates_for_calibration(phase1_dir: Path) -> float:
    """
    Load actual gene coordinates from Phase 1 to calibrate maximum gene span.
    Returns the 95th percentile of gene lengths in kb.
    """
    global CALIBRATED_MAX_GENE_SPAN_KB

    try:
        coords_file = phase1_dir / "cluster_gene_coordinates.tsv"
        if coords_file.exists():
            df = pd.read_csv(coords_file, sep='\t')
            if 'length' in df.columns and len(df) > 0:
                # Calculate 95th percentile of gene lengths
                gene_lengths_kb = df['length'] / 1000.0
                percentile_95 = gene_lengths_kb.quantile(0.95)
                # Add 20% buffer for safety
                calibrated = percentile_95 * 1.2
                print(f"[Phase4] Calibrated max gene span from actual genes: {calibrated:.1f} kb (95th percentile: {percentile_95:.1f} kb)")
                CALIBRATED_MAX_GENE_SPAN_KB = calibrated
                return calibrated
    except Exception as e:
        print(f"[Phase4] Warning: Could not load gene coordinates for calibration: {e}")

    # Fallback to default if calibration fails
    print(f"[Phase4] Using default max gene span: {CALIBRATED_MAX_GENE_SPAN_KB} kb")
    return CALIBRATED_MAX_GENE_SPAN_KB
    ...
    for (scaffold, strand), group in df.groupby(["scaffold", "strand"]):
        ...
        for start_idx in range(len(sg_sorted)):
            ...
            if cov >= min_cluster_coverage:
                ...
                # Use calibrated max gene span based on actual gene coordinates from Phase 1
                # This prevents filtering out valid long genes (e.g., 20kb genes with introns)
                if cov > 0.95 and span_kb > CALIBRATED_MAX_GENE_SPAN_KB:
                    # Skip this mega-cluster - it's probably bridging tandem array
                    print(f"[Phase4] Filtering cluster with span {span_kb:.1f} kb > calibrated max {CALIBRATED_MAX_GENE_SPAN_KB:.1f} kb")
                    continue
...
def main() -> None:
    ...
    # Load actual gene coordinates from Phase 1 to calibrate clustering parameters
    # This prevents filtering out valid long genes (e.g., 20kb genes with introns)
    load_gene_coordinates_for_calibration(args.phase1_dir)
