...
from constants import lookup_protein
...
def main():
    ...
    parser.add_argument("--phase5b-dir", type=Path, default=None,
                        help="Phase 5b novel loci output directory (optional)")
    ...
    novel_loci_count = 0
    novel_failed_count = 0

    if args.phase5b_dir and args.phase5b_dir.exists():
        novel_clustered_file = args.phase5b_dir / "novel_loci_clustered.tsv"
        novel_candidates_file = args.phase5b_dir / "novel_candidates.tsv"

        if novel_clustered_file.exists() and novel_candidates_file.exists():
            print("\n[2b] Extracting novel loci targets...")

            # Load clustered loci (locus assignments)
            novel_clusters_df = pd.read_csv(novel_clustered_file, sep='\t')
            print(f"  Loaded {len(novel_clusters_df)} novel loci")

            # Load novel candidates with full target info
            novel_candidates_df = pd.read_csv(novel_candidates_file, sep='\t')

            # For each novel locus, find the best target in each member genome
            # The representative tells us the scaffold/position pattern
            for _, locus_row in novel_clusters_df.iterrows():
                locus_name = locus_row['locus_name']
                locus_id = locus_row['locus_id']
                representative = locus_row['representative']
                member_genomes = locus_row.get('member_genomes', '').split(';') if pd.notna(locus_row.get('member_genomes', '')) else []

                if not representative.startswith('NOVEL_'):
                    continue

                # Extract expected scaffold from representative
                # Format: NOVEL_{genome}_{scaffold}_{position}
                rep_parts = representative.replace('NOVEL_', '').rsplit('_', 1)
                rep_scaffold_part = rep_parts[0].rsplit('_', 1)[-1] if len(rep_parts) > 1 else ''

                # For each member genome, find the best matching candidate
                for genome in member_genomes:
                    # Get candidates from this genome
                    genome_candidates = novel_candidates_df[novel_candidates_df['genome'] == genome]

                    if genome_candidates.empty:
                        continue

                    # Pick the candidate with highest bitscore
                    # (all candidates from a member genome should be valid, pick best)
                    best_candidate = genome_candidates.loc[genome_candidates['best_bitscore'].idxmax()]

                    target_id = best_candidate['target_gene_id']
                    scaffold = best_candidate['scaffold']
                    start = int(best_candidate['start'])
                    end = int(best_candidate['end'])
                    strand = best_candidate.get('strand', '+')

                    # Create output directory: {output}/{genome}/NOVEL_{locus_name}/
                    novel_locus_dir = args.output_dir / genome / f"NOVEL_{locus_name}"
                    novel_locus_dir.mkdir(parents=True, exist_ok=True)
                    ...
                    extracted_length, length_flag = write_protein_fasta(
                        output_file, target_id, protein_seq,
                        scaffold, start, end, strand,
                        query_length_aa=0  # No reference for novel loci
                    )
                    novel_loci_count += 1
                    extracted_count += 1
            ...
            if novel_failed_count > 0:
                print(f"  Failed: {novel_failed_count} novel targets")
        else:
            print("\n[2b] No novel loci files found in Phase 5b directory")
    else:
        print("\n[2b] Skipping novel loci (no Phase 5b directory)")
                        ...
                        if unplaced_dir.is_dir():
                            for protein_file in unplaced_dir.glob("*_protein.fasta"):
                                for record in SeqIO.parse(protein_file, 'fasta'):
                                    ...
                                    protein_count += 1
                # Handle NOVEL_ directories (novel loci)
                elif target_dir.name.startswith('NOVEL_'):
    ...
    print(f"  - Unplaceable: {len(unplaceable_df)}")
    print(f"  - Novel loci: {novel_loci_count}")
    print(f"Failed: {failed_count + novel_failed_count} targets")
