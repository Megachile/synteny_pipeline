...
# Consolidated utility: scaffold normalization (if needed later)
...
class Phase2SyntenyComparator:
    """Compare loci based on flanking gene matches (count-based).

    No sliding window or percentage thresholds; only count unique query matches.
    """
    ...
    def compare_flanking(self, flanking_file1, flanking_file2, locus1_name, locus2_name):
        ...
        percent1 = result1['matches'] / result1['total'] if result1['total'] > 0 else 0
        percent2 = result2['matches'] / result2['total'] if result2['total'] > 0 else 0

        # Add percent to results
        result1['percent'] = percent1
        result2['percent'] = percent2
    ...
    def _search_direction(self, query_file, target_file, label):
        ...
        try:
            ...
            if query_file.stat().st_size == 0 or target_file.stat().st_size == 0:
                return {
                    'matches': 0,
                    'total': 0,
    ...
    def classify_relationship(self, comparison, same_genome, is_tandem1, is_tandem2):
        """Classify based on match count only (>=3 considered syntenic)."""

        has_synteny = comparison['matches'] >= 3
...
def main():
    ...
    print(f"Synteny criterion: unique flanking matches ≥ 3 (no percent/window gating)")
    ...
    # Compare all pairs
    print("\n[1] Comparing all locus pairs (count-based ≥3 threshold)...")
    comparisons = []
    ...
    for i, locus1 in loci_df.iterrows():
        for j, locus2 in loci_df.iterrows():
            ...
            # Determine if passes global threshold (≥3 matches)
            passes_global = result['matches'] >= 3

            # No window-based filtering in this version
            passes_window = False
            window_matches = 0
            window_pos = None
            ...
            print(f"    {result['matches']}/{result['total']} matches", end='')
            print(f" | threshold: {'OK' if result['matches'] >= 3 else '<3'}")
