...
def create_gene_family_matrix(gene_family, loci_list, all_targets_df, synteny_blocks_df, species_map, phylo_order_map, seq_metadata, output_dir):
    ...
    # Syntenic loci (from loci_list) + two unplaceable categories
    locus_categories = loci_list + [f"{gene_family}_near_block", f"{gene_family}_confidently_distinct"]
    ...
    for genome in all_genomes:
        ...
        for target in genome_targets:
            ...
            # For unplaceable targets, use confidence to determine category
            placement = target.get('placement', '')
            confidence = target.get('confidence', '')
            if placement == 'unplaceable':
                if confidence == 'near_block':
                    assigned_to = f"{gene_family}_near_block"
                elif confidence == 'confidently_distinct':
                    assigned_to = f"{gene_family}_confidently_distinct"
            ...
            if length > 0:
                ...
                # Unplaceable targets don't have extracted sequences
                target_str = "hit"
...
def main():
    ...
    for gene_family in gene_families:
        ...
        if not matrix_df.empty:
            ...
            # Count syntenic vs unplaceable (split by confidence)
            syntenic_count = 0
            near_block_count = 0
            confidently_distinct_count = 0

            for col in family_loci:
                syntenic_count += len(matrix_df[matrix_df[col] != ""])

            near_block_col = f"{gene_family}_near_block"
            if near_block_col in matrix_df.columns:
                near_block_count = len(matrix_df[matrix_df[near_block_col] != ""])

            distinct_col = f"{gene_family}_confidently_distinct"
            if distinct_col in matrix_df.columns:
                confidently_distinct_count = len(matrix_df[matrix_df[distinct_col] != ""])

            total_count = syntenic_count + near_block_count + confidently_distinct_count
            if total_count > 0:
                print(f"    Syntenic: {syntenic_count} ({syntenic_count/total_count*100:.1f}%)")
                print(f"    Ambiguous (near_block): {near_block_count} ({near_block_count/total_count*100:.1f}%)")
                print(f"    Distinct (confidently_distinct): {confidently_distinct_count} ({confidently_distinct_count/total_count*100:.1f}%)")
