...
def main():
    ...
    parser.add_argument(
        "--min-query-coverage", type=float, default=0.3,
        help="Minimum query coverage to keep a hit (default: 0.3 = 30%%). "
             "Lower values capture partial domain matches which may be false positives."
    )
    ...
    for genome_id in genomes:
        ...
        # Check for DIAMOND database - first in genome dir, then in diamond_db_dir
        db_in_genome_dir = genome_dir / f"{genome_id}_helixer.dmnd"
        db_in_output_dir = diamond_db_dir / f"{genome_id}_helixer"

        if db_in_genome_dir.exists():
            # Use pre-built database from ragtag_output
            db_path = genome_dir / f"{genome_id}_helixer"
        elif (db_in_output_dir.parent / f"{db_in_output_dir.name}.dmnd").exists():
            # Use database from output dir
    ...
    if all_results:
        ...
        # Filter by minimum query coverage to remove partial domain matches
        # (e.g., venom serine protease matching generic proteases via catalytic domain only)
        n_before_filter = len(output_df)
        output_df = output_df[output_df['combined_query_coverage'] >= args.min_query_coverage]
        n_after_filter = len(output_df)
        n_filtered = n_before_filter - n_after_filter
        if n_filtered > 0:
            print(f"\n  Query coverage filter (>={args.min_query_coverage*100:.0f}%): "
                  f"{n_before_filter} → {n_after_filter} ({n_filtered} low-coverage hits removed)")
