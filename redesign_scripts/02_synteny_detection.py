...
def process_locus(row: pd.Series, genome_dbs: dict[str, Path], out_dir: Path,
    ...
    # Build query order from flanking file
    q_order = _build_query_order_map(flanking_file)
    ...
    for genome_name, genome_db in genome_dbs.items():
        ...
        if xml_path.exists() and is_valid_blast_xml(xml_path):
            ...
            ok = run_tblastn(flanking_file, genome_db, xml_path, evalue=evalue, max_targets=max_targets, threads=threads)
...
def main():
    ...
    for db in sorted(args.genome_db_dir.glob('*.nhr')):
        genome_dbs[db.stem] = args.genome_db_dir / db.stem
    print(f"Genomes: {len(genome_dbs)}")
