...
def classify_targets(targets_df: pd.DataFrame, blocks_df: pd.DataFrame,
                     ...
                     rescue_min_evalue: float = 1e-50,
                     primary_assignment_kb: int = 500,
                     alternative_max_kb: int = 200) -> pd.DataFrame:
    ...
    for _, t in targets_df.iterrows():
        ...
        # TIER 1: Try parent locus first with generous threshold
        # TIER 2: Only consider alternatives if parent fails AND alternative is close

        parent_locus = locus_id  # This is the locus that detected the target
        all_blocks = by_scaf.get((genome, scf), [])

        # Helper function to find best block match
        def find_best_block(blocks, max_dist_kb):
            best = None
            best_overlap = -1
            best_dist = None
            ...
            for b in blocks:
                ...
                if ov > 0:
                    if ov > best_overlap:
                        best = b; best_overlap = ov; best_dist = 0
                    elif ov == best_overlap and best is not None:
                        # Tie-breaker: closer center
                        t_c = (s + e) // 2
                        b_c = (bs + be) // 2
                        best_c = (int(best['start']) + int(best['end'])) // 2
                        if abs(t_c - b_c) < abs(t_c - best_c):
                            best = b; best_dist = 0
                    continue
        ...
        # TIER 1: Try parent locus with generous threshold
        parent_blocks = [b for b in all_blocks if str(b.get('locus_id')) == parent_locus]
        parent_match, parent_dist = find_best_block(parent_blocks, max_dist_kb=primary_assignment_kb)

        chosen = None
        assignment_tier = 'unplaceable'

        if parent_match is not None:
            parent_dist_kb = parent_dist / 1000.0 if parent_dist is not None else 0

            if parent_dist_kb <= primary_assignment_kb:
                # Accept parent assignment
                chosen = parent_match
                assignment_tier = 'primary'
            else:
                # Parent is too far, check alternatives
                other_blocks = [b for b in all_blocks if str(b.get('locus_id')) != parent_locus]
                alt_match, alt_dist = find_best_block(other_blocks, max_dist_kb=alternative_max_kb)

                if alt_match is not None:
                    alt_dist_kb = alt_dist / 1000.0 if alt_dist is not None else 0

                    if alt_dist_kb <= alternative_max_kb:
                        # Alternative is close, use it
                        chosen = alt_match
                        assignment_tier = 'alternative'
                    else:
                        # Both far, fall back to parent
                        chosen = parent_match
                        assignment_tier = 'primary_distant'
                else:
                    # No alternative, use distant parent
                    chosen = parent_match
                    assignment_tier = 'primary_distant'
        else:
            # No parent blocks, check alternatives
            other_blocks = [b for b in all_blocks if str(b.get('locus_id')) != parent_locus]
            alt_match, alt_dist = find_best_block(other_blocks, max_dist_kb=alternative_max_kb)
        ...
        best_dist = 0 if chosen and assignment_tier in ['primary', 'alternative'] else None
        ...
        if chosen is not None:
            ...
            chosen_block_start = ''
            chosen_block_end = ''
            # Initial reasons for unplaceable classification (before rescue)
            if not all_blocks:
                unplaceable_reason = "no_blocks_on_scaffold"
            else:
                # Parent blocks exist but no acceptable match within thresholds
                if parent_blocks and parent_match is None:
                    unplaceable_reason = "no_parent_block_within_window"
                elif not parent_blocks:
                    unplaceable_reason = "no_parent_block_and_no_alternative"
                else:
                    # Parent/alternative too distant from target
                    unplaceable_reason = "too_far_from_all_blocks"
        ...
        if placement == 'unplaceable' and rescue_same_scaffold:
            ...
            if parent_blocks:
                ...
                if best_d is not None and best_d <= rescue_upper_kb * 1000 and ev <= rescue_min_evalue:
                    ...
                    rescue_flag = True
                    rescue_dist = best_d
                    # Successfully rescued → clear unplaceable reason
                    unplaceable_reason = ""
                else:
                    # Rescue failed; refine reason based on distance / e-value if not already set
                    if best_d is not None and best_d > rescue_upper_kb * 1000:
                        if not unplaceable_reason:
                            unplaceable_reason = "beyond_rescue_distance"
                    elif best_d is not None and ev > rescue_min_evalue:
                        if not unplaceable_reason:
                            unplaceable_reason = "evalue_too_weak_for_rescue"
        ...
        out['assignment_tier'] = assignment_tier
        # Only set reason for final unplaceable targets; syntenic rows get blank.
        out['reason_for_unplaceable'] = unplaceable_reason if placement == 'unplaceable' else ""
        rows.append(out)
...
def parse_args():
    ...
    p.add_argument('--primary-assignment-kb', type=int, default=500, help='Max distance (kb) for primary assignment to parent locus')
    p.add_argument('--alternative-max-kb', type=int, default=200, help='Max distance (kb) for alternative locus assignment')
    return p.parse_args()
...
def main():
    ...
    classified = classify_targets(targets_df, blocks_df, locus_defs,
                                  ...
                                  primary_assignment_kb=args.primary_assignment_kb,
                                  alternative_max_kb=args.alternative_max_kb)
    ...
    sel_rows = []
    if not syn.empty and {'assigned_block_id','locus_id','genome'} <= set(syn.columns):
        # Count assignments per block
        counts = syn.groupby(['locus_id','genome','assigned_block_id']).size().reset_index(name='n_targets')
