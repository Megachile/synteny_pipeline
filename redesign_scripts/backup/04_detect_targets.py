...
def cluster_hits_for_locus(hits: List[Dict], min_split_gap_kb: int) -> List[Dict]:
    ...
    for (scf, strand), arr in by_key.items():
        ...
        for seg_s, seg_e in segs:
            ...
            if not seg_hits:
                continue
            # Distinct queries contributing to this segment
            qids = sorted({str(h.get('query_id', '')) for h in seg_hits if h.get('query_id')})
            # Best query by lowest evalue, then highest bitscore
            best = None
            for h in seg_hits:
                if best is None:
                    best = h
                    continue
                if h.get('evalue', float('inf')) < best.get('evalue', float('inf')):
                    best = h
                elif h.get('evalue', float('inf')) == best.get('evalue', float('inf')):
                    if h.get('bitscore', -1) > best.get('bitscore', -1):
                        best = h
            out.append({
                'scaffold': scf,
                'strand': strand,
                ...
                'query_best_id': str(best.get('query_id')) if best else '',
                'query_best_evalue': float(best.get('evalue')) if best else None,
            })
    return out
        ...
        for locus_id, lh in by_locus.items():
            ...
            for c in clusters:
                all_out_rows.append({
                    ...
                    # carry query provenance forward to Phases 5/6
                    'query_id': c.get('query_best_id', ''),
                    'query_best_evalue': c.get('query_best_evalue', ''),
                    'query_ids': c.get('query_ids', ''),
